...
import lab5.threads.RandomThread;
import lab5.threads.TxtThread;
import lab5.threads.XmlThread;
...
public class Runner {
    ...
    public static void main(String[] args) {
        System.out.format("%9s|%8s| %30s| %10s| %8s| %8s|\n", "Thread", "Код", "Имя", "Дата рожд.", "Отряд", "Комната");
        Runnable randomThread = new RandomThread();
        Runnable txtThread = new TxtThread();
        Runnable xmlThread = new XmlThread();
        ExecutorService service = Executors.newFixedThreadPool(3);
        service.execute(randomThread);
        service.execute(txtThread);
        service.execute(xmlThread);
